...
fn sum_decimal(
    ...
    match (lhs, rhs) {
        (None, None) => ScalarValue::Decimal128(None, Some(*precision), Some(*scale)),
        (None, rhs) => {
            ScalarValue::Decimal128(rhs.clone(), Some(*precision), Some(*scale))
        }
        (lhs, None) => {
            ScalarValue::Decimal128(lhs.clone(), Some(*precision), Some(*scale))
        }
        (lhs, rhs) => ScalarValue::Decimal128(
    ...
    Ok(match (lhs, rhs) {
        (ScalarValue::Decimal128(v1, p1, s1), ScalarValue::Decimal128(v2, p2, s2)) => {
            if p1.eq(p2) && s1.eq(s2) {
                // TODO the result data type should use the new precision and scale
                sum_decimal(v1, v2, &p1.unwrap(), &s1.unwrap())
            } else {
                return Err(DataFusionError::Internal(format!(
...
mod tests {
    ...
    fn sum_decimal() -> Result<()> {
        ...
        assert_eq!(
            ScalarValue::Decimal128(Some(123 + 124), Some(10), Some(2)),
            result
        );
        // negative test
        let left = ScalarValue::Decimal128(Some(123), Some(10), Some(2));
        let right = ScalarValue::Decimal128(Some(124), Some(11), Some(2));
        let result = sum(&left, &right);
        assert_eq!(
        ...
        assert_eq!(ScalarValue::Decimal128(Some(15), Some(10), Some(0)), result);
        ...
        generic_test_op!(
            ...
            Sum,
            ScalarValue::Decimal128(Some(15), Some(10), Some(0)),
            DataType::Decimal(10, 0)
        )
    }
    ...
    fn sum_decimal_with_nulls() -> Result<()> {
        // test sum
        let left = ScalarValue::Decimal128(None, Some(10), Some(2));
        let right = ScalarValue::Decimal128(Some(123), Some(10), Some(2));
        let result = sum(&left, &right)?;
        assert_eq!(
            ScalarValue::Decimal128(Some(123), Some(10), Some(2)),
            result
        );
        ...
        let array: ArrayRef = Arc::new(decimal_builder.finish());
        let result = sum_batch(&array)?;
        assert_eq!(ScalarValue::Decimal128(Some(13), Some(10), Some(0)), result);
        ...
        generic_test_op!(
            ...
            DataType::Decimal(10, 0),
            Sum,
            ScalarValue::Decimal128(None, Some(10), Some(0)),
            DataType::Decimal(10, 0)
        )
    }
    ...
    fn sum_decimal_all_nulls() -> Result<()> {
        // test sum
        let left = ScalarValue::Decimal128(None, Some(10), Some(2));
        let right = ScalarValue::Decimal128(None, Some(10), Some(2));
        let result = sum(&left, &right)?;
        assert_eq!(ScalarValue::Decimal128(None, Some(10), Some(2)), result);
